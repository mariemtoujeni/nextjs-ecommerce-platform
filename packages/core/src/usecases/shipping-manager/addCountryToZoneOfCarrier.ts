...
export const addCountryToZoneOfCarrierUseCase = async (zone: string, country: string, carrier: string) : Promise<ShipmentZoneCountry[]> => {
    ...
    const newCountryList = await shipmentConf.addCountryToZoneOfCarrier(zone, countryCode.code, carrier);
